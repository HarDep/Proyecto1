package x.y.z.configs;

import java.awt.*;

public class GlobalConfigs {
    public static final Dimension TOTAL_DIMENSION = new Dimension(1000,700);
    protected static final String PATH_CONFIGS_PROPERTIES = "resources/config.properties";
    protected static final String PATH_LANGUAGE_ENGLISH = "resources/en.properties";
    protected static final String PATH_LANGUAGE_SPANISH = "resources/es.properties";
}
